...
void The_oto::Xoa_tt_oto(string id_xoa) {
	fstream f;
	string str[1000];
	int n = 0;
	...
	f.open("So_xe_vao_oto.txt", ios::out | ios::trunc);
	if (!f.is_open())
	{
		cout << "Khong the mo file!\n";
		system("pause");
...
void The_oto::Nhap_oto() {
	...
	cout << "+ Vi tri: " << Vi_tri.Lay_ten_slot(id) << endl;
	...
	do
	{
		...
		{
			fstream f;
			f.open("So_xe_vao_oto.txt", ios::out | ios::app);
			f << endl << id << "|";
			f << Bien_so << "|";
			f << Vi_tri.Lay_ten_slot(id) << "|";
			f << Gio_vao << "|";
			f << Phut_vao << "|";
			f << Giay_vao << "|";
			f << Ngay_vao << "|";
	...
	for (int i = 0; i < n; i++)
	{
		if (str[i] == id)
		{
			...
			do
			{
				...
				if (luachon == "1")
				{
					f.open("So_xe_ra_oto.txt", ios::out | ios::app);
					if (!f.is_open())
					{
						cout << "Khong the mo file!\n";
						system("pause");
...
void The_oto::Tim_kiem_xe() {
	...
	// Kiem tra bien so trong so xe ra oto neu oto khong co trong bai
	bool check2 = false;
	if (check == false)
	{
	...
	if (check == true) // Tim kiem xe trong bai
	{
		...
		else
		{
			while (!f.eof())
			{
				...
				s[m].erase(0, 1);
				getline(f, s[m + 1], '|');
				if (s[m + 1] == tt_tim_kiem || s[m] == tt_tim_kiem)
				{
					getline(f, s[m + 2], '|');
					getline(f, s[m + 3], '|');
					getline(f, s[m + 4], '|');
					...
					cout << "\t+ Vi tri: " << s[m + 2] << endl;
					cout << "\t+ Thoi gian gui:\n";
					cout << "\t\t- Gio vao: " << s[m + 3] << " : " << s[m + 4] << " : " << s[m + 5] << endl;
					cout << "\t\t- Ngay vao: " << s[m + 6] << " - " << s[m + 7] << " - " << s[m + 8] << endl;
					cout << "\t+ Tai khoan phu trach: " << s[m + 9] << endl;
			...
			while (!f.eof())
			{
				...
				if (s1[y + 1] == tt_tim_kiem || s1[y] == tt_tim_kiem)
				{
					...
					if (ngay - a < 2)
					{
						...
						cout << "\t+ Vi tri: " << s1[y + 2] << endl;
						cout << "\t+ Thoi gian gui:\n";
						cout << "\t\t- Gio vao: " << s1[y + 3] << " : " << s1[y + 4] << " : " << s1[y + 5] << endl;
						cout << "\t\t- Ngay vao: " << s1[y + 6] << " - " << s1[y + 7] << " - " << s1[y + 8] << endl;
						cout << "\n\t+ Thoi gian ra:\n";
						cout << "\t\t- Gio ra: " << s1[y + 9] << " : " << s1[y + 10] << " : " << s1[y + 11] << endl;
						cout << "\t\t- Ngay ra: " << s1[y + 12] << " - " << s1[y + 13] << " - " << s1[y + 14] << endl;
						cout << "\t- Thanh tien: " << s1[y + 15] << endl;
						cout << "\t- Tai khoan phu trach: " << s1[y + 16] << endl;
					}
				}
				else
				{
...
void The_oto::Xoa_so_xe_ra() {
	fstream f;
	f.open("So_xe_ra_oto.txt", ios::out | ios::trunc);
	if (!f.is_open())
	{
		cout << "Khong the mo file!\n";
		system("pause");
